...
#define ISR_ZLIB      0x1000    /* compress output with zlib */
#endif
...
typedef struct isreport {       /* --- an item set reporter --- */
  ...
  #ifdef USE_ZLIB               /* if to write compressed output */
  z_stream   zsets;             /* stream for compressing item sets */
  z_stream   ztids;             /* stream for compressing trans. ids */
  UCHAR      *zbuf;             /* output buffer for compression */
  #endif
...
extern int       isr_xinfo    (ISREPORT *rep, RSUPP supp,
                               ...
                               RSUPP salt, RSUPP halt, RSUPP join);
extern void      isr_getinfo  (ISREPORT *rep, CCHAR *sel, double *vals);
#ifdef ISR_CLOMAX
