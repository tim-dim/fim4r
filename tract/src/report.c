/*----------------------------------------------------------------------
  ...
  History : 2008.08.18 item set reporter created in tract.[ch]
            ...
            2017.05.30 optional compression with zlib library added
----------------------------------------------------------------------*/
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
#ifdef USE_ZLIB
#define isr_flush(r)   isr_flushx(r, Z_NO_FLUSH)
#define isr_finish(r)  isr_flushx(r, Z_FINISH)
static void isr_flushx (ISREPORT *rep, int flush)
#else
...
{                               /* --- flush the output buffer */
  ...
  if (rep->mode & ISR_ZLIB) {   /* if to compress the output */
    size_t n;                   /* number of bytes to write */
    rep->zsets.next_in  = (UCHAR*)rep->buf;
    rep->zsets.avail_in = (unsigned)(rep->next -rep->buf);
    do {                        /* compress and write output */
      rep->zsets.avail_out = BS_WRITE;
      rep->zsets.next_out  = rep->zbuf;
      deflate(&rep->zsets, flush);
      n = (size_t)(BS_WRITE -rep->zsets.avail_out);
      fwrite(rep->zbuf, sizeof(UCHAR), n, rep->file);
    } while (rep->zsets.avail_out == 0);
  } else                        /* while output buffer becomes empty */
  #endif                        /* (i.e. more data can be compressed) */
...
#ifdef USE_ZLIB
#define isr_tidflush(r)   isr_tidflushx(r, Z_NO_FLUSH)
#define isr_tidfinish(r)  isr_tidflushx(r, Z_FINISH)
static void isr_tidflushx (ISREPORT *rep, int flush)
#else
...
{                               /* --- flush the output buffer */
  ...
  if (rep->mode & ISR_ZLIB) {   /* if to compress the output */
    size_t n;                   /* number of bytes to write */
    rep->ztids.next_in  = (UCHAR*)rep->tidbuf;
    rep->ztids.avail_in = (unsigned)(rep->tidnxt -rep->tidbuf);
    do {                        /* compress and write output */
      rep->ztids.avail_out = BS_WRITE;
      rep->ztids.next_out  = rep->zbuf;
      deflate(&rep->ztids, flush);
      n = (size_t)BS_WRITE -rep->ztids.avail_out;
      fwrite(rep->zbuf, sizeof(UCHAR), n, rep->tidfile);
    } while (rep->ztids.avail_out == 0);
  } else                        /* while output buffer becomes empty */
  #endif                        /* (i.e. more data can be compressed) */
...
{                               /* --- create an item set reporter */
  ...
  #ifdef USE_ZLIB               /* if to use optional compression */
  rep->zbuf    = NULL;          /* clear the compression buffer */
  #endif
  rep->occs    = NULL;
  rep->tids    = NULL;
  rep->tidcnt  = 0;
...
{                               /* --- delete an item set reporter */
  ...
  #ifdef USE_ZLIB               /* if to use optional compression */
  if (rep->zbuf)   free(rep->zbuf);
  #endif                        /* delete a compression buffer */
...
{                               /* --- open an output file */
  ...
  fastchk(rep);                 /* check for fast output */
  #ifdef USE_ZLIB               /* if optional output compression */
  if (file                      /* if to write to a file */
  && (rep->mode & ISR_ZLIB)) {  /* and to compress the output */
    rep->zsets.zalloc = Z_NULL; /* clear allocation and free functions */
    rep->zsets.zfree  = Z_NULL;
    rep->zsets.opaque = Z_NULL; /* create a compression buffer */
    if (!rep->zbuf) rep->zbuf = (UCHAR*)malloc(BS_WRITE *sizeof(UCHAR));
    if (!rep->zbuf) return E_NOMEM;
    if (deflateInit(&rep->zsets, Z_DEFAULT_COMPRESSION) != Z_OK)
      return E_NOMEM;           /* initialize the compression */
...
{                               /* --- close the output file */
  ...
  #ifdef USE_ZLIB               /* if optional output compression */
  if (rep->mode & ISR_ZLIB) deflateEnd(&rep->zsets);
  #endif                        /* clean up the compression stream */
...
{                               /* --- set/open trans. id output file */
  ...
  && (rep->mode & ISR_ZLIB)) {  /* and to compress the output */
    ...
    rep->ztids.zfree  = Z_NULL;
    rep->ztids.opaque = Z_NULL; /* create a compression buffer */
    if (!rep->zbuf) rep->zbuf = (UCHAR*)malloc(BS_WRITE *sizeof(UCHAR));
    if (!rep->zbuf) return E_NOMEM;
    if (deflateInit(&rep->ztids, Z_DEFAULT_COMPRESSION) != Z_OK)
      return E_NOMEM;           /* initialize the compression */
  }                             /* (default method: deflate) */
  #endif
  return 0;                     /* return 'ok' */
...
{                               /* --- close trans. id output file */
  ...
  #ifdef USE_ZLIB               /* if optional output compression */
  if (rep->mode & ISR_ZLIB) deflateEnd(&rep->ztids);
  #endif                        /* clean up the compression stream */
